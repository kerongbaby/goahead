...
#if 0
    websWriteSocket(wp, outbuf, sizeof(outbuf));
#else
...
#if 0
    int ret = websFlush(wp, 0 /* none-block */);
    if(ret < 0) {
        logmsg(2, "wsPing failed?");
    } else if(ret == 0)
        logmsg(2, "wsPing not finished?");
#endif
    websDone(wp);
#endif
}
...
bool parseWebsocketIncoming(Webs *wp) {
    ...
    char        *end, c;
    rxbuf = &wp->rxbuf;
    logmsg(2, "parseWebsocketIncoming, buflen: %d\r\n", bufLen(rxbuf));
    if(bufLen(rxbuf) <= 0)
        return 0;
    int cc;
    evhtp_ws_parser p;
...
#else
                    while((z = bufGetc(wp)) >= 0) {
                        int           j = p.content_idx % 4;
                        unsigned char xformed_oct;

                        xformed_oct     = (p.frame.masking_key & __MASK[j]) >> __SHIFT[j];
                        bufInsertc(wp, ((unsigned char)(z & 0xff) ^ xformed_oct));
                        p.content_idx += 1;
                    }
#endif
#if 0
...
#else
                logmsg(2, "websockets - hook code: %02X, content length: %d", p.frame.hdr.opcode, p.content_len);
#endif
